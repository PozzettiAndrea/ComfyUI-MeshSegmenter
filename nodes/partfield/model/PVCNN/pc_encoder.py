...
def create_pointnet_components(
    ...
    for out_channels, num_blocks, voxel_resolution in blocks:
        ...
        if voxel_resolution is None:
            block = functools.partial(SharedMLP, device=device, dtype=dtype, operations=operations)
        else:
...
# merge surface sampled PC and rendering backprojected PC (w/ 2D features):
    def __init__(self, in_channels=204, device=None, dtype=None, operations=None):
        super(PCMerger, self).__init__()
        self.mlp_normal = SharedMLP(3, [128, 128], device=device, dtype=dtype, operations=operations)
        self.mlp_rgb = SharedMLP(3, [128, 128], device=device, dtype=dtype, operations=operations)
        self.mlp_sam = SharedMLP(204 - 6, [128, 128], device=device, dtype=dtype, operations=operations)
...
class PVCNNEncoder(nn.Module):
    def __init__(self, pvcnn_feat_dim, device=None, dtype=None, operations=None, in_channels=3, use_2d_feat=False):
        ...
        if self.use_2d_feat:
            self.merger = PCMerger(device=device, dtype=dtype, operations=operations)
