...
def conv3x3(in_channels, out_channels, stride=1,
            padding=1, bias=True, groups=1, device=None, dtype=None, operations=None):
...
def upconv2x2(in_channels, out_channels, mode='transpose', device=None, dtype=None, operations=None):
    if operations is None:
        operations = nn
    if mode == 'transpose':
        ...
        return nn.Sequential(
            ...
            conv1x1(in_channels, out_channels, device=device, dtype=dtype, operations=operations))

def conv1x1(in_channels, out_channels, groups=1, device=None, dtype=None, operations=None):
    if operations is None:
        operations = nn
    return operations.Conv2d(
        in_channels,
...
def conv1x1triplane3daware(in_channels, out_channels, order='xz', device=None, dtype=None, operations=None, **kwargs):
    return ConvTriplane3dAware(
        lambda inp, out: conv1x1(inp, out, device=device, dtype=dtype, operations=operations, **kwargs),
        in_channels, out_channels, order=order)

def Normalize(in_channels, num_groups=32, device=None, dtype=None, operations=None):
    if operations is None:
        operations = nn
    num_groups = min(in_channels, num_groups)  # avoid error if in_channels < 32
    return operations.GroupNorm(num_groups=num_groups, num_channels=in_channels, eps=1e-6, affine=True, device=device, dtype=dtype)
...
class Upsample(nn.Module):
    def __init__(self, in_channels, with_conv, device=None, dtype=None, operations=None):
        super().__init__()
        if operations is None:
            operations = nn
        self.with_conv = with_conv
        if self.with_conv:
            self.conv = operations.Conv2d(in_channels,
                                        in_channels,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1,
...
class ResnetBlock3dAware(nn.Module):
    def __init__(self, in_channels, out_channels=None, device=None, dtype=None, operations=None):
        super().__init__()
        if operations is None:
            operations = nn
        self.in_channels = in_channels
        ...
        self.norm1 = Normalize(in_channels, device=device, dtype=dtype, operations=operations)
        self.conv1 = conv3x3(self.in_channels, self.out_channels, device=device, dtype=dtype, operations=operations)

        self.norm_mid = Normalize(out_channels, device=device, dtype=dtype, operations=operations)
        self.conv_3daware = conv1x1triplane3daware(self.out_channels, self.out_channels, device=device, dtype=dtype, operations=operations)

        self.norm2 = Normalize(out_channels, device=device, dtype=dtype, operations=operations)
        self.conv2 = conv3x3(self.out_channels, self.out_channels, device=device, dtype=dtype, operations=operations)

        if self.in_channels != self.out_channels:
            self.nin_shortcut = operations.Conv2d(in_channels,
                                                out_channels,
                                                kernel_size=1,
                                                stride=1,
                                                padding=0,
...
class DownConv3dAware(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, downsample=True, with_conv=False,
        ...
        self.block = ResnetBlock3dAware(in_channels=in_channels,
            out_channels=out_channels, device=device, dtype=dtype, operations=operations)

        self.do_downsample = downsample
        self.downsample = Downsample(out_channels, with_conv=with_conv, device=device, dtype=dtype, operations=operations)
...
class UpConv3dAware(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.upsample = Upsample(in_channels, with_conv, device=device, dtype=dtype, operations=operations)

        if self.merge_mode == 'concat':
            self.norm1 = Normalize(in_channels+out_channels, device=device, dtype=dtype, operations=operations)
...
class UNetTriplane3dAware(nn.Module):
    def __init__(self, out_channels, in_channels=3, depth=5,
        ...
        if use_initial_conv:
            self.conv_initial = conv1x1(self.in_channels, self.start_filts, device=device, dtype=dtype, operations=operations)
        ...
        for i in range(depth-1):
            ...
            up_conv = UpConv3dAware(ins, outs,
                merge_mode=merge_mode, device=device, dtype=dtype, operations=operations)
        ...
        self.norm_out = Normalize(outs, device=device, dtype=dtype, operations=operations)
        self.conv_final = conv1x1(outs, self.out_channels, device=device, dtype=dtype, operations=operations)
...
def setup_unet(output_channels, input_channels, unet_cfg, device=None, dtype=None, operations=None):
