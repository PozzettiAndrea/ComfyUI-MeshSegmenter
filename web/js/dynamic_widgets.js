...
console.log("[MeshSegmenter DynamicWidgets] Loading extension...");
...
async function loadMappings() {
    const paths = [
        "/extensions/ComfyUI-MeshSegmenter/js/backend_mappings.json",
        "/extensions/comfyui-meshsegmenter/js/backend_mappings.json",
    ];
    ...
    for (const path of paths) {
        try {
            ...
            if (response.ok) {
                ...
                console.log("[MeshSegmenter DynamicWidgets] Loaded mappings from", path);
                return true;
            }
        } catch (e) {
            // Try next path
    ...
    console.warn("[MeshSegmenter DynamicWidgets] Could not load backend_mappings.json");
    return false;
}
...
function updateWidgetVisibility(node, selectedBackend) {
    ...
    console.log(`[MeshSegmenter DynamicWidgets] ${nodeClass}: backend="${selectedBackend}", visible=[${visibleWidgets.join(', ')}]`);

    let visibilityChanged = false;

    for (const widget of node.widgets || []) {
        // Skip the backend selector itself and common inputs
        ...
        if (isBackendWidget) {
            ...
            if (wasHidden !== widget.hidden) {
                ...
                console.log(`[MeshSegmenter DynamicWidgets] Widget "${widget.name}": hidden=${widget.hidden}`);
            }
        }
    }
...
app.registerExtension({
    name: "MeshSegmenter.DynamicWidgets",

    nodeCreated(node) {
        const nodeClass = node.comfyClass;
        if (!BACKEND_MAPPINGS[nodeClass]) return;

        console.log(`[MeshSegmenter DynamicWidgets] Setting up node: ${nodeClass}`);

        const backendWidgetName = BACKEND_WIDGET_NAME[nodeClass];
        const backendWidget = node.widgets?.find(w => w.name === backendWidgetName);

        if (!backendWidget) {
            console.log(`[MeshSegmenter DynamicWidgets] No ${backendWidgetName} widget found`);
            return;
        }
...
console.log("[MeshSegmenter DynamicWidgets] Extension registered");
