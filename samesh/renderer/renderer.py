...
class Renderer:
    ...
    def render(
        ...
        def render_faces(faces: NumpyTensor['h w 3']) -> NumpyTensor['h w']:
            ...
            # Check for invalid face IDs and clamp them to background
            num_faces = len(self.tmesh.faces)
            valid_mask = (faces >= -1) & (faces < num_faces)
            invalid_count = (~valid_mask).sum()
            if invalid_count > 0:
                # Clamp invalid face IDs to -1 (background) - this happens at mesh edges
                faces = np.where(valid_mask, faces, -1)
        ...
        def render_matte(
            ...
            if interpolate_norms: # NOTE requires process=True
                # Handle background (-1) and invalid faces by clamping to valid index
                faces_flat = faces.reshape(-1)
                num_mesh_faces = len(self.tmesh.faces)

                # Create a safe index array - clamp negatives to 0 for indexing, will mask later
                safe_faces = np.clip(faces_flat, 0, num_mesh_faces - 1)

                verts_index = self.tmesh.faces[safe_faces]           # (n, 3)
...
if __name__ == '__main__':
    from PIL import Image
    from ..data.loaders import read_mesh, read_scene, remove_texture, scene2mesh
    from ..models.shape_diameter_function import shape_diameter_function, colormap_shape_diameter_function, prep_mesh_shape_diameter_function
