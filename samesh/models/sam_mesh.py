...
from ..data.loaders import read_scene, remove_texture, scene2mesh
from ..renderer.renderer import Renderer, render_multiview, colormap_faces, colormap_norms
from .sam import SamModel, Sam2Model, combine_bmasks, colormap_mask, remove_artifacts, point_grid_from_mask
from ..utils.cameras import *
from ..utils.mesh import duplicate_verts
from .shape_diameter_function import *
...
class SamModelMesh(nn.Module):
    ...
    def render(self, scene: Scene, visualize_path=None) -> dict[str, NumpyTensor]:
        ...
        if self.config.cache is not None and self.config.cache.exists():
            if self.config.cache_overwrite:
                print('    Cache exists but overwrite enabled, regenerating...')
        ...
        bmasks_list = []
        modes_used = self.config.sam_mesh.use_modes
        print(f'    Using modes: {modes_used}')

        if 'norms' in modes_used:
            print('    Processing NORMALS mode...')
            images1 = [colormap_norms(norms) for norms in renders['norms']]
            bmasks1 = [
                call_sam(image, faces != -1) for image, faces in \
                    tqdm(zip(images1, renders['faces']), '      SAM on normals', total=len(images1))
            ]
            bmasks_list.extend(bmasks1)
            print(f'      Got {sum(len(m) for m in bmasks1)} masks from normals')

        if 'sdf' in modes_used:
            print('    Processing SDF (Shape Diameter Function) mode...')
            print('      Computing shape diameter function...')
            ...
            self.load(tmesh_sdf)
            print('      Rendering SDF views...')
            renders_sdf = render_func(uv_map=True)
            images2 = renders_sdf['matte']

            bmasks2 = [
                call_sam(image, faces != -1) for image, faces in \
                    tqdm(zip(images2, renders['faces']), '      SAM on SDF', total=len(images2))
            ]
            bmasks_list.extend(bmasks2)
            renders['sdf'] = renders_sdf['matte']
            print(f'      Got {sum(len(m) for m in bmasks2)} masks from SDF')

        if 'matte' in modes_used:
            print('    Processing MATTE mode...')
            images3 = renders['matte']
            bmasks3 = [
                call_sam(image, faces != -1) for image, faces in \
                    tqdm(zip(images3, renders['faces']), '      SAM on matte', total=len(images3))
            ]
            bmasks_list.extend(bmasks3)
            print(f'      Got {sum(len(m) for m in bmasks3)} masks from matte')
    ...
    def lift(self, renders: dict[str, NumpyTensor]) -> dict:
        ...
        print(f'    Computing face2label for each view ({mp.cpu_count()} CPU cores)...')
        ...
        print(f'    Building match graph ({mp.cpu_count()} CPU cores)...')
        args = []
    ...
    def smooth(self, face2label_consistent: dict) -> dict:
        ...
        # fill islands
        print(f'    Filling holes ({self.config.sam_mesh.smoothing_iterations} iterations)...')
    ...
    def smooth_repartition_faces(self, face2label_consistent: dict, target_labels=None) -> dict:
        ...
        if target_labels is None:
            print(f'    Running graph-cut optimization (lambda={lambda_seed})...')
            ...
            final_labels = len(np.unique(refined_partition))
            print(f'    Repartitioned into {final_labels} segments')
            return {
        ...
        print(f'    Repartitioned: {cur_labels} labels (target: {target_labels}) using lambda={lambdas[cur_lambda_index]:.3f} in {cur_iteration} iterations')
    ...
    def forward(self, scene: Scene, visualize_path=None, target_labels=None, return_renders=False) -> tuple[dict, Trimesh]:
        """
        Run the full segmentation pipeline.

        Args:
            scene: Input mesh/scene
            visualize_path: Path to save visualization images
            target_labels: Target number of segments
            return_renders: If True, also returns the renders dict

        Returns:
            If return_renders=False: (face2label dict, trimesh)
            If return_renders=True: (face2label dict, trimesh, renders dict)
        """
        print('  [3a/7] Loading scene into renderer...')
        self.load(scene)
        print(f'    Scene loaded with {len(self.renderer.tmesh.faces)} faces')

        print('')
        print('  [3b/7] Rendering multiview images + computing SAM masks...')
        renders = self.render(scene, visualize_path=visualize_path)
        print(f'    Generated {len(renders["faces"])} views')

        print('')
        print('[4/7] Lifting 2D masks to 3D (building match graph)...')
        face2label_consistent = self.lift(renders)
        print(f'    Labeled {len(face2label_consistent)} faces')

        print('')
        print('[5/7] Smoothing labels (removing small components)...')
        ...
        if return_renders:
            return face2label_consistent, self.renderer.tmesh, renders
...
def segment_mesh(filename: Path | str, config: OmegaConf, visualize=False, extension='glb', target_labels=None, texture=False, return_renders=False) -> Trimesh:
    """
    Segment a mesh using SAM2.

    Args:
        filename: Path to input mesh file
        config: OmegaConf configuration
        visualize: Whether to save visualization images
        extension: Output mesh format
        target_labels: Target number of segments (-1 for auto)
        texture: Whether to keep texture
        return_renders: If True, returns (mesh, renders_dict) instead of just mesh

    Returns:
        If return_renders=False: Trimesh colored mesh
        If return_renders=True: (Trimesh colored mesh, dict with render data)
    """
    print('=' * 60)
    print('SAMesh: Starting mesh segmentation')
    print('=' * 60)
    ...
    print('[1/7] Initializing SAMesh model...')
    model = SamModelMesh(config)
    print('  Model initialized!')

    print('[2/7] Loading mesh...')
    tmesh = read_mesh(filename, norm=True)
    print(f'  Mesh loaded: {len(tmesh.vertices)} vertices, {len(tmesh.faces)} faces')

    if not texture:
        print('  Removing texture (using vertex colors)...')
    ...
    print('')
    print('[3/7] Running segmentation pipeline...')
    faces2label, _, renders = model(tmesh, visualize_path=visualize_path, target_labels=target_labels, return_renders=True)
    ...
    output_mesh_path = f'{config.output}/{filename.stem}_segmented.{extension}'
    output_json_path = f'{config.output}/{filename.stem}_face2label.json'

    tmesh_colored.export(output_mesh_path)
    print(f'  Saved mesh: {output_mesh_path}')

    json.dump(faces2label, open(output_json_path, 'w'))
    print(f'  Saved labels: {output_json_path}')

    num_segments = len(set(faces2label.values()))
    print('')
    print('=' * 60)
    print(f'SAMesh: Segmentation complete! Found {num_segments} segments')
    print('=' * 60)

    if return_renders:
        return tmesh_colored, renders
